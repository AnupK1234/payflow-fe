...
export class ClientDashboard implements OnInit {
  ...
  pieChartData = false;       // ✅ For template *ngIf
  monthlyChartData = false;   // ✅ For template *ngIf
  ...
  loadDashboard(): void {
    ...
    // Recent requests
    this.clientService.getRecentRequests(this.clientId).subscribe({
      next: (res) => (this.recentRequests = res),
        ...
        datasets: [
          {
            ...
            data: Array(12).fill(stats.paid), // placeholder, replace with real monthly data
